...
public class UserController {
    ...
    @GetMapping("/dni/laboratorista/{dni}")
    public ResponseEntity<UserEntity> buscarLaboratoristaPorDni(@PathVariable String dni) {
        TypeUser tipoLaboratorista = TypeUser.LABORATORISTA;
        return userService.buscarPorDniYTipo(dni, tipoLaboratorista)
                .map(ResponseEntity::ok)
                .orElse(ResponseEntity.notFound().build());
    }

    @GetMapping("/apellidos")
    public ResponseEntity<List<UserEntity>> buscarMedicosPorApellido(@RequestParam String apellido) {
        TypeUser tipoMedico = TypeUser.MEDICO;
        List<UserEntity> resultados = userService.buscarPorApellidoYTipo(apellido, tipoMedico);
